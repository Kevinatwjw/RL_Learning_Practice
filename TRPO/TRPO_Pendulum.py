...
from TRPO_Class import TRPO_Continuous
...
if __name__ == "__main__":
    ...
    state_dim = env.observation_space.shape[0]                              # 环境观测维度
    action_range = env.action_space.shape[0]                            # 环境动作空间大小
    num_episodes = 2000
    hidden_dim = 128
    gamma = 0.98
    lmbda = 0.95
    ...
    # build agent
    agent = TRPO_Continuous(state_dim, hidden_dim, action_range, lmbda, kl_constraint, alpha, critic_lr, gamma, device)
    ...
    os.makedirs('./TRPO_PPO/result', exist_ok=True)
    ...
    plt.title(f'{agent._get_name()} on Pendulum-v1 with {reward_type} reward')
    # 显示图例
    plt.legend()
    # 保存图像到 result 目录
    plt.savefig(f'./TRPO_PPO/result/{agent._get_name()}({reward_type}).png')
    # 显示图像
    plt.show()   
